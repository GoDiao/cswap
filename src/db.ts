...
import { loadConfig, configToProviders } from './config.js';
...
/**
 * Get all providers from both CC Switch database and cswap config.
 * cswap config providers take precedence on name conflicts.
 */
export async function getAllProviders(): Promise<Provider[]> {
  const ccSwitchAvailable = isDbAvailable();
  const config = await loadConfig();
  const configProviders = configToProviders(config);
  ...
  const configNames = new Set(
    configProviders.map((p) => p.name.toLowerCase())
  );

  // CC Switch providers that don't conflict with cswap config
  const merged = [
    ...configProviders,
    ...ccProviders.filter(
      (p) => !configNames.has(p.name.toLowerCase())
    ),
  ];
